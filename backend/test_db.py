    ...
    def update_enrollment_status(self, enrollment_id: str, new_status: str) -> bool:
        ...
        try:
            ...
            if result.modified_count > 0:
                ...
                enrollment = self.db_config.db.enrollments.find_one({'_id': ObjectId(enrollment_id)})
                if enrollment:
...
if __name__ == "__main__":
    ...
    # Example: Store a contact form submission
    contact_form = {
        'name': 'John Doe',
        'email': 'johndoe@example.com',
        'phone': '1234567890',
        'course_interest': 'Python Programming',
        'message': 'I would like to know more about your Python course.'
    }
    success, doc_id = db_ops.store_form_submission(contact_form)
    print(f"Contact form stored? {success}, ID: {doc_id}")

    # Example: Store an enrollment
    enrollment_data = {
        ...
        'payment_status': 'paid',
        'additional_notes': 'Looking forward to the course!'
    }
    success, enroll_id = db_ops.store_enrollment(enrollment_data)
    print(f"Enrollment stored? {success}, ID: {enroll_id}")

    # Update enrollment status
    if enroll_id:
        status_updated = db_ops.update_enrollment_status(enroll_id, 'confirmed')
